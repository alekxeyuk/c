...
static void parse_args(int argc, char* argv[], char** ifile, char** ofile,
                       uint32_t* psize) {
  int opt;
  ...
  while ((opt = getopt(argc, argv, "f:o:p:h")) != -1) {
    switch (opt) {
      ...
      case 'p':
        ...
        *psize = (uint16_t)atoi(optarg);
        if (*psize == 0) err(EXIT_FAILURE, "piece_size is invalid.\n");
        break;
      case 'h':
        fprintf(stderr,
                "Usage: %s [-f input_file] [-o output_file] [-p piece_size]\n"
                "Or use parse mode for file parsing\n",
                argv[0]);
        exit(EXIT_SUCCESS);
    }
  }
...
static char* output_file_name(const char* input_file) {
  ...
  char* output_file = malloc(base_len + 9);  // .torrent + \0
  if (!output_file) {
...
static void parse(char* file_name) {
...
int main(int argc, char* argv[]) {
  ...
  if (!input_file) {
    fprintf(stderr,
            RED "Error: Input file is required. Use -h for help\n" RESET);
    exit(EXIT_FAILURE);
  }
  if (!output_file) {
  ...
  length = get_file_size(input_file);
  piece_count = (length + piece_length - 1) / piece_length;
  ...
  // Prepare memory for pieces hashes
  ...
  u_char base64[HASH_SIZE + 1 + 2];
  for (uint32_t i = 0; i < piece_count; i++) {
    uint32_t offset = i * piece_length;
    uint32_t chunk_size =
        (i == piece_count - 1) ? (length - offset) : piece_length;
