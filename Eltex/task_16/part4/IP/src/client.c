...
static void send_ip_packet(const char *packet, size_t p_len, int fd, const char *dest_ip, uint16_t dest_port) {
...
int main(int argc, char *argv[]) {
  ...
  while (!should_exit) {
    sleep(1);
    send_ip_packet(ip_packet, ip_len, sockfd, dest_ip, dest_port);
    for (;;) {
