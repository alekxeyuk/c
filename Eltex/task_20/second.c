...
#define DEVICE_NAME "secondchardev"
#define BUF_LEN 31
...
static ssize_t device_read(struct file *fd, char __user *buffer, size_t length, loff_t *offset) {
  // Can't use read_lock here, 'cause it causes simple_read_from_buffer to stop correctly working -> it always returns bad address error
