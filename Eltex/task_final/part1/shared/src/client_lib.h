...
typedef int (*client_receive_response_t)(ClientSession* session, char* bufer, size_t bufer_size);
typedef int (*client_disconnect_t)(ClientSession* session);

typedef struct {
  client_init_t init;
  client_connect_t connect;
  client_send_message_t send_message;
  client_receive_response_t receive_response;
  client_disconnect_t disconnect;
} operations_t;
